...
fake = 'BY : minsZGONZ & MinsBoTz' //ganti sesuka lu
numbernye = '0'
waktu = '-'
alasan = '-'
//=================================================//
...
case 'bcgc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						bcgc = await minsZ.downloadMediaMessage(encmedia)
						for (let _ of groupMembers) {
							minsZ.sendMessage(_.jid, bcgc, image, {caption: `* BROADCAST GROUP *\n*Group* : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							minsZ.sendMessage(_.jid, bc, image, {caption: `[ Izin Broadcast ]\n\n${body.slice(4)}`})
						}
...
case 'asupan':
			...
			minsZ.updatePresence(from, Presence.composing) 
				...
				case 'nsfwneko':
				    try {
						if (!isNsfw) return reply(' *fitur nsfw belum di aktifkan* ')
						res = await fetchJson(`https://herokuapp.com/api/nsfwneko?apikey=${Minsbothero}`, {method: 'get'})
						buffer = await getBuffer(res.result)
						minsZ.sendMessage(from, buffer, image, {quoted: mek, caption: 'ni anjim'})
					} catch (e) {
...
*Chapter Akhir* : ${komik.chapter_akhir}`
    ...
    case 'pinterest':
                    ...
                    reply('wait')
					minsZ.updatePresence(from, Presence.composing) 
					if (err) return minsZ.sendMessage(from, fs.readFileSync('./stik/not-pound.jpg'), image)
...
- *Link Dev* : ${i.link_dev}\n❉─────────────────────❉`
    ...
    case 'fitnah':
            ...
            var bot = gh.split("|")[2];
            minsZ.sendMessage(from, `${bot}`, text, {quoted: { key: { fromMe: false, participant: `${mentioned}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target}` }}})
            break
    case 'settarget':
    ...
    case 'fitnahpc':
            ...
            const responye = await minsZ.sendMessage(jids, `${split[1]}`, MessageType.text, options)
            await minsZ.deleteMessage(jids, { id: responye.messageID, remoteJid: jids, fromMe: true })
            break
    case 'tomp3':
