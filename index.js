...
prefix = '.'
//=================================================//
module.exports = minS = async (minS, mek) => {
	try {
		...
		const q = args.join(' ')
		const botNumber = minS.user.jid
		const botNumberss = minS.user.jid + '@c.us'
		const isGroup = from.endsWith('@g.us')
		let sender = isGroup ? mek.participant : mek.key.remoteJid
		// const isSelfNumber = config.NomorSELF
		// const isOwner = sender.id === isSelfNumber
		const totalchat = await minS.chats.all()
		const groupMetadata = isGroup ? await minS.groupMetadata(from) : ''
        ...
        const conts = mek.key.fromMe ? minS.user.jid : minS.contacts[sender] || { notify: jid.replace(/@.+/, '') }
        const pushname = mek.key.fromMe ? minS.user.name : conts.notify || conts.vname || conts.name || '-'
...
responseButton = (type == 'listResponseMessage') ? mek.message.listResponseMessage.title : ''
        ...
        const costum = (pesan, tipe, target, target2) => {
			minS.sendMessage(from, pesan, tipe, {quoted: { key: { fromMe: false, participant: `${target}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target2}` }}})
			}

        const reply = (teks) => {
            minS.sendMessage(from, teks, text, {quoted:mek})
        }

        const sendMess = (hehe, teks) => {
            minS.sendMessage(hehe, teks, text)
        }

        const mentions = (teks, memberr, id) => {
            (id == null || id == undefined || id == false) ? minS.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : minS.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": memberr } })
        }

        const fakestatus = (teks) => {
            minS.sendMessage(from, teks, text, {
                quoted: {
                    key: {
                        fromMe: false,
        ...
        const fakethumb = (teks, yes) => {
            minS.sendMessage(from, teks, image, {thumbnail:fs.readFileSync('./stik/fake.jpeg'),quoted:mek,caption:yes})
        }
...
const Minsbotlol = 'f30a10449621c4fbe804359e'
        ...
        const sendStickerFromUrl = async(to, url) => {
                ...
                download(url, './stik' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${filess} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${asw}`, (err) => {
                        ...
                        minS.sendMessage(to, media, MessageType.sticker,{quoted:mek})
        ...
        const sendMediaURL = async(to, url, text="", mids=[]) =>{
                ...
                download(url, filename, async function () {
                    ...
                    minS.sendMessage(to, media, type, { quoted: mek, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
		...
		minS.updatePresence(from, Presence.composing)
		var Kick = `${sender.split("@")[0]}@s.whatsapp.net`
		setTimeout( () => {
		reply('byee')
		}, 1100)
		setTimeout( () => {
		minS.groupRemove(from, [Kick]).catch((e) => {reply(`*ERROR:* ${e}`)}) 
					}, 1000)
		setTimeout( () => {
...
//FUNCTION
        ...
        addafk(mek.key.remoteJid)
        heheh = ms(Date.now() - waktu)
        minS.sendMessage(mek.key.remoteJid,`@${owner} Sedang Offline!\n\n *Alasan :* ${alasan}\n *Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik lalu\n\nSilahkan Hubungi Lagi Nanti`, MessageType.text,{contextInfo:{ mentionedJid: [`${owner}@s.whatsapp.net`],'stanzaId': "B826873620DD5947E683E3ABE663F263", 'participant': "0@s.whatsapp.net", 'remoteJid': 'status@broadcast', 'quotedMessage': {"imageMessage": {"caption": "*OFFLINE*", 'jpegThumbnail': fs.readFileSync('./stik/thumb.jpeg')}}}})
          }
        }
            }
          }
...
//========================================================================================================================//
		...
		const isQuotedSticker = type === 'extendedTextMessage' && content.includes('stickerMessage')
         // Button Cmd By HiRyn
					...
					minS.sendMessage(from, `「 *SUCCES OPEN GRUP* 」`, MessageType.text)
					minS.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (responseButton === 'CLOSE') {
					await minS.groupSettingChange(from, GroupSettingChange.messageSend, true)
					minS.sendMessage(from, `「 *SUCCES CLOSE GRUP* 」`, MessageType.text)
					}
...
||► _${prefix}fdeface_
||► _${prefix}emoji_
||► _${prefix}tahta_ <contoh: ${prefix}Farel ganteng>
...
||
❏ *</FUN>* ❏
||► _${prefix}fitnah_ <contoh: ${prefix}fitnah @taguser|p|p>
||► _${prefix}fitnahpc_
||► _${prefix}kontak_
||► _${prefix}suit_ < batu, gunting, kertas >
||► _${prefix}dare_
||► _${prefix}truth_
...
||► _${prefix}nulis_ <contoh: ${prefix}nulis farel>
||► _${prefix}bisakah_
||► _${prefix}kapankah_
||► _${prefix}apakah_
||► _${prefix}cekcantik_
||► _${prefix}gantengcek_
||► _${prefix}pantun_ <random>
||► _${prefix}darkjokes_ <random>
||► _${prefix}pbucin_
||► _${prefix}ocr_
||
||
...
||► _${prefix}ssweb_ <contoh : https:/google.com>
...
||► _${prefix}ping_
||► _${prefix}inspect_
||► _${prefix}join_ <link grup whatsApp>
...
case 'antilink':
			...
			case 'tagme':
			     if (!isGroup) return reply (mess.only.group)
				await costum(`@${sender.split('@')[0]}`, text, minS, `@${sender.split('@')[0]}`)
				break
case 'request':
				case 'requestfitur':
				
                     const request = body.slice(8)
                      if (request.length > 300) return minS.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', msgType.text, {quoted: mek})
                    ...
                    minS.sendMessage('6289653007306@s.whatsapp.net', options, text, {quoted: mek})
...
case 'clone':
					...
					try {
						pp = await minS.getProfilePicture(id)
						buffer = await getBuffer(pp)
						minS.updateProfilePicture(botNumber, buffer)
...
case 'archive':
                ...
                await sleep(3000)
                minS.modifyChat(from, ChatModification.archive)
                break
case 'unarchiveall':
                if (!mek.key.fromMe) return reply('*Kamu Owner?*')
                reply('*succes unarchive all chat*')
                console.log('succes unarchive chat = ' + from)
                anu = await minS.chats.all()
                for (let _ of anu) {
                minS.modifyChat(_.jid, ChatModification.unarchive)
                }
                break
case 'unpin':
                if (!mek.key.fromMe) return reply('*Kamu Owner?*')
                minS.modifyChat(from, ChatModification.unpin)
...
case 'report':
                     ...
                     const pesan = body.slice(8)
                      if (pesan.length > 300) return minS.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', msgType.text, {quoted: mek})
                        var nomor = mek.participant
                       const rprt = `*[REPORT]*\nNomor : @${nomor.split("@s.whatsapp.net")[0]}\nPesan : ${pesan}`
...
case 'ocr':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						const media = await minS.downloadAndSaveMediaMessage(encmedia)
						reply(mess.wait)
...
case 'bcgc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						bcgc = await minS.downloadMediaMessage(encmedia)
						for (let _ of groupMembers) {
							minS.sendMessage(_.jid, bcgc, image, {caption: `* BROADCAST GROUP *\n*Group* : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						bc = await minS.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							minS.sendMessage(_.jid, bc, image, {caption: `[ Izin Broadcast ]\n\n${body.slice(4)}`})
						}
...
case 'asupan':
			...
			minS.updatePresence(from, Presence.composing) 
				try {
				minS.sendMessage(from, fs.readFileSync('./stik/protokol.jpg'), image)
                await sleep(1000)
				...
				minS.sendMessage(from, asupan, video, {quoted: mek, caption: '```ASUPAN NIH:V```'})
				} catch {
				...
				case 'nsfwneko':
				    try {
						...
						minS.sendMessage(from, buffer, image, {quoted: mek, caption: 'ni anjim'})
					} catch (e) {
...
case 'grup':
					...
					if (args[0] === 'buka') {
						...
						minS.groupSettingChange(from, GroupSettingChange.messageSend, true)
					}
					break
case 'pbucin':
				if (args.length < 1) return reply('Mana Nama?')
				minS.sendMessage(from, fs.readFileSync('./stik/ok.jpg'), image)
				rate = body.slice(8)
				await sleep(10000)
				persenbucin = ["4%\n\nHadehh","9%\n\nMasih Kecil Dah Bucin Ae","17%\n\nNakk Masih Kecil","28%\n\nYoalahh hmm","34%\n\nMayan Lah","48%\n\nGatau","59%\n\nBiasa Kang Bucin","62%\n\n HadehhhðŸƒ","74%\n\n bucen Teroosss","83%\n\n SekaliÂ² kek Ga bucin Gitu","97%\n\nHadehh PakboiÂ²","100%\n\nHadehhh Ini Bukan Bucin Tapi Pakboi","29%\n\nKasian Mana Masih Muda","94%\n\n Dasar Pakboi","75%\n\n Ya Ampun"]
				const pbucin = persenbucin[Math.floor(Math.random() * persenbucin.length)]
				minS.sendMessage(from, 'Persen Bucin Kak: *'+rate+'*\n\nJawaban : '+ pbucin +'', text, { quoted: mek })
				break 
case 'darkjokes':
if (!mek.key.fromMe) return reply('fitur ini masih belum bisa')
				reply(mess.wait)
                 ...
                 minS.sendMessage(from, darkjokes, image, {quoted: mek, caption: '\`\`\`DARK JOKES\`\`\`'})
				break  
case 'pantun':
			try {
				minS.sendMessage(from, fs.readFileSync('./stik/ok.jpg'), image)
			anu = await fetchJson(`https://api.zeks.xyz/api/pantun?apikey=${ZeksApi}`, {method: 'get'})
			minS.sendMessage(from, `${anu.result.pantun}`, text, {quoted: mek})
			} catch {
					reply(mess.error.bug)
					}
			break  
...
case 'toptt':
					...
					media = await minS.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp3')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('Gagal mengkonversi audio ke ptt')
						topt = fs.readFileSync(ran)
						minS.sendMessage(from, topt, audio, {mimetype: 'audio/mp4', quoted: mek, ptt:true})
						})
					break
case 'cekcantik':
				case 'cantikcek':
						...
						minS.sendMessage(from, 'Pertanyaan : Cantik Cek Kakak *'+cantik+'*\n\nPersen Kecantikan : '+ tik +'', text, { quoted: mek })
						break
				case 'gantengcek':
				case 'cekganteng':
				case 'tampancek':
						...
						minS.sendMessage(from, 'Pertanyaan : Cek Ganteng Bang *'+ganteng+'*\n\nJawaban : '+ teng +'', text, { quoted: mek })
						break
case 'hobby':
           if (!isGroup) return reply('khusus grup')
					...
					minS.sendMessage(from, 'Pertanyaan : *'+hobby+'*\n\nJawaban : '+ by, text, { quoted: mek })
					break
case 'readall':
                if (!mek.key.fromMe) return reply('*Kamu Owner?*')
                var chats = await minS.chats.all()
                chats.map( async ({ jid }) => {
                await minS.chatRead(jid)
                })
		reply('`\`\`\`!!sukses!! read semua chat !\`\`\``')
		break
case 'pin':
                if (!mek.key.fromMe) return reply('*Kamu Owner?*')
                minS.modifyChat(from, ChatModification.pin)
...
minS.sendMessage(from, hasil, text, {
  quoted: mek
})
break	
case 'spamcall':
					...
					minS.sendMessage(from, `${anu.result.logs}`, text, {quoted: mek})
					break
case 'mengkick':	
case 'kick':
...
minS.sendMessage(from, {quoted: mek, sendEphemeral: true})
					...
					if (mentioned.length > 1) {
						...
						minS.groupRemove(from, mentioned)
					} else {
...
case 'demote':
					...
					if (mentioned.length > 1) {
						...
						minS.groupDemoteAdmin(from, mentioned)
					}
					break
...
${res.desc}` : '\n*Tidak ada Deskripsi*'}
`
minS.sendMessage(from, todd, text, {quoted: mek, contextInfo: { mentionedJid: parseMention(todd)}})
break
    case 'delete':
					case 'd':
					if (!isGroup)return reply(mess.only.group)
					minS.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
					break
case 'listonline':
        		let ido = args && /\d+\-\d+@g.us/.test(args[0]) ? args[0] : from
			    let online = [...Object.keys(minS.chats.get(ido).presences), minS.user.jid]
			    minS.sendMessage(from, 'List Online:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join`\n`, text, { quoted: mek,
...
case 'bisakah':
					...
					minS.sendMessage(from, 'Pertanyaan : *'+bisakah+'*\n\nJawaban : '+ keh, text, { quoted: mek })
					break
				case 'kapankah':
				    case 'kapan':
...
case 'tagall':	
            ...
            case 'admin':				
                  ...
                  if (!isBotGroupAdmins) return reply (' jadikan bot sebagai admin ')
				    if (!isGroup) return reply('fitur ini khusus grup')
				    linkgc = await minS.groupInviteCode (from)
				    yeh = `https://chat.whatsapp.com/${linkgc}\n\nlink Group *${groupName}*`
				    minS.sendMessage(from, yeh, text, {quoted: mek})
					break
            case 'leave': 
				if (!isGroup) return reply(mess.only.group)
				if (!mek.key.fromMe) return reply('khusus owner')
				anu = await minS.groupLeave(from, `Bye All Member *${groupMetadata.subject}*`, groupId)
				break
case 'nulis':
				case 'tulis':
				...
				minS.sendMessage(from, buff, image, {quoted: mek, caption: 'Lebih baik nulis sendiri ya kak :*'})
				break
case 'promote':
					...
					if (mentioned.length > 1) {
						...
						minS.groupMakeAdmin(from, mentioned)
					}
					break
 case 'setname':
                if (!groupAdmins && !mek.key.fromMe) return reply(' fitur ini khusus owner dan admin ')
                ...
                if (!isGroup) return reply('fitur ini khusus grup')
                minS.groupUpdateSubject(from, `${body.slice(9)}`)
                minS.sendMessage(from, '⟪ SUKSES ⟫ Mengubah Nama Grup', text, {quoted: mek})
					break
                case 'setdesc':
                if (!groupAdmins && !mek.key.fromMe) return reply(' fitur ini khusus owner dan admin ')
                if (!isBotGroupAdmins) return reply(' jadikan bot sebagai admin ')		
                if (!isGroup) return reply('fitur ini khusus grup')
                if (args.length < 1) return reply('error')
                minS.groupUpdateDescription(from, `${body.slice(9)}`)
                minS.sendMessage(from, '⟪ SUKSES ⟫ Mengubah Desk Grup', text, {quoted: mek})
					break
case 'sarananime':
                    fakestatus('*_wait_*')
					...
					minS.sendMessage(from, srn, image, { quoted: mek, caption: '*saran anime*\n'+ saran })
					break
case 'seberapagay':			
					gay = body.slice(13)
...
case 'quotes':		
				...
				minS.sendMessage(from, ''+tes+'\n\n_By : minSin Panutanque._', text, { quoted: mek })
				break
case 'setppbot':
					...
					media = await minS.downloadAndSaveMediaMessage(enmedia)
					await minS.updateProfilePicture(botNumber, media)
...
case 'wallpegunungan':  
                   ...
                   minS.sendMessage(from, anu3, image, {caption: `nihh kack`, quoted: mek})
                   break
case 'wallpaper':   
                   case 'walpaper':  
                   reply(minS)
                   ...
                   minS.sendMessage(from, anu, image, {caption: `nihh kack`, quoted: mek})
                   break
            case 'owner':
                 minS.sendMessage(from, {displayname: "Jeff", vcard: vcard}, MessageType.contact, { quoted: mek})
                 minS.sendMessage(from, 'Jika Mau Save Chat Aja Gan Ntar Disave Back:)',text, { quoted: mek} )
                 break
case 'gitcreator':
...
case 'nekonime':
           ...
           minS.sendMessage(from, hasil, image, {quoted: mek})
           break
case 'kicktime':
					if (!isGroup) return reply(mess.only.group)
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					setTimeout( () => {
					minS.sendMessage(from, 'Yok Sama" kita mengenang teman kita yg telah terkick', text)
					}, 8000)
					setTimeout( () => {
					...
					setTimeout( () => {
					minS.sendMessage(from, `dengan tuhan yg maha pengasih lagi maha penyayang maka kamu akan saya kick  @${mentioned[0].split('@')[0]}`, text) // ur cods
					}, 5000)
					setTimeout( () => {
					minS.sendMessage(from, 'yahahha gw kick lu ajg', text)
					}, 2500)
					setTimeout( () => {
...
minS.sendMessage(from, pbt, image, {quoted: mek})
break
...
minS.sendMessage(from, pbn, image, {quoted: mek})
break
case 'tupai':
					pai = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					tup = await minS.downloadAndSaveMediaMessage(pai)
					ran = getRandom('.mp3')
					exec(`ffmpeg -i ${tup} -filter:a "atempo=0.5,asetrate=65100" ${ran}`, (err, stderr, stdout) => {
						fs.unlinkSync(tup)
						if (err) return reply('Error!')
						hah = fs.readFileSync(ran)
						minS.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek})
						fs.unlinkSync(ran)
					})
				break
				case 'gemuk':
					muk = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					gem = await minS.downloadAndSaveMediaMessage(muk)
					ran = getRandom('.mp3')
...
case 'tahta':
				...
				minS.sendMessage(from, bh, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				break
case 'dare':   
					...
					minS.sendMessage(from, tod, image, { quoted: mek, caption: '*Dare*\n\n'+ der })
					break
    case 'truth':
					...
					minS.sendMessage(from, truteh, image, { caption: '*Truth*\n\n'+ ttrth, quoted: mek })
					break
case 'ss': 
case 'ssweb':
		try {
      ...
      reply(mess.wait)
			...
			minS.sendMessage(from, eses, image, {caption: q, quoted: mek, thumbnail: Buffer.alloc(0)})
			} catch (e) {
				e = String(e)
...
reply('╔═════════════════ \n ║ *DONASI UNTUK ZFAZ* \n ╠═════════════════ \n ║╭──❉ *DONASI SEMAMPUNYA* ❉ \n ║│➸ *TRI*: _089653007306_ \n ║│➸ *TELKOMSEL*: _085211545148_ \n ║│➸ *OVO*: _089653007306_ \n ║│➸ *DANA*: _089653007306_ \n ║│➸ *GOPAY*: _085211545148_ \n ║│➸ *SAWERIA*: _https://saweria.co/Farl_ \n ║│➸ *TRAKTEER*: _https://trakteer.id/far-gaming-exe-yexd1_ \n  ║╰──────────────── \n ╠═════════════════ \n ║tri sama telkomsel masukin pulsa aj:v  \n ╠═════════════════ \n ║         minS-HX-BOT \n ║  ▌│█║▌║▌║║▌║▌║█│▌ \n ║  ▌│█║▌║▌║║▌║▌║█│▌ \n ║         minS-HX-BOT \n ╠═════════════════ \n ║ _*POWERED BY ZFAZ*_ \n ╚═════════════════')
                  break	
    case 'list':
					  let po = minS.prepareMessageFromContent(from, {
				"listMessage":{
            ...
            minS.relayWAMessage(po, {waitForAck: true, quoted: mek})
			break
...
break
    ...
    case 'igstory': 
            ...
            for(let i of result.medias){
                if(i.url.includes('mp4')){
                    ...
                    minS.sendMessage(from,link,video,{quoted: mek,caption: `Type : ${i.type}`})
                } else {
                    let link = await getBuffer(i.url)
                    minS.sendMessage(from,link,image,{quoted: mek,caption: `Type : ${i.type}`})                  
                }
            }
            });
            break
    case 'caripesan':
            ...
            let v = await minS.searchMessages(q,from,10,1)
            let s = v.messages
            ...
            await minS.sendMessage(from,'Nih pesannya',text,{quoted:el[i]})
            }
            } catch(e){
    ...
    case 'lirik':
            ...
            reply(minS.sendMessage(from, fs.readFileSync('./stik/ok.jpg'), image))
...
${anime.desc}\n\n*Link Batch* : ${anime.batch}\n*Link Download SD* : ${anime.batchSD}\n*Link Download HD* : ${anime.batchHD}`
            ...
            minS.sendMessage(from,ram,image,{quoted:mek,caption:rem})
            break
    case 'komiku':
...
*Chapter Akhir* : ${komik.chapter_akhir}`
    ...
    case 'chara':
            ...
            await minS.sendMessage(from,li,image,{quoted: mek})
            break
    case 'pinterest':
                    if (!q) return minS.sendMessage(from, fs.readFileSync('./stik/not-pound.jpg'), image)
                    reply('wait')
					minS.updatePresence(from, Presence.composing) 
					if (err) return minS.sendMessage(from, fs.readFileSync('./stik/not-pound.jpg'), image)
					data = await fetchJson(`https://fdciabdul.tech/api/pinterest?keyword=${body.slice(11)}`, {method: 'get'})
					n = JSON.parse(JSON.stringify(data));
					nimek =  n[Math.floor(Math.random() * n.length)];
					pok = await getBuffer(nimek)
					minS.sendMessage(from, pok, image, { quoted: mek, caption: `*⟪ PINTEREST ⟫*`})
					break
    case 'playstore':
...
- *Link Dev* : ${i.link_dev}\n❉─────────────────────❉`
    ...
    case 'kontag':
            ...
            + 'END:VCARD'.trim()
            minS.sendMessage(from, {displayName: `${nah}`, vcard: vcard}, contact, {contextInfo: {"mentionedJid": members_ids}})
            break
    case 'sticktag':
            if ((isMedia && !mek.message.videoMessage || isQuotedSticker) && args.length == 0) {
            encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
            file = await minS.downloadAndSaveMediaMessage(encmedia, filename = getRandom())
            value = args.join(" ")
            var group = await minS.groupMetadata(from)
            ...
            minS.sendMessage(from, ini_buffer, sticker, options)
            fs.unlinkSync(file)
            } else {
    ...
    case 'totag':
            ...
            ini_buffer = fs.readFileSync(file)
            minS.sendMessage(from, ini_buffer, image, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            minS.sendMessage(from, ini_buffer, audio, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            minS.sendMessage(from, ini_buffer, video, options)
            fs.unlinkSync(file)
        } else{
    ...
    case 'fitnah':
            ...
            var bot = gh.split("|")[2];
            minS.sendMessage(from, `${bot}`, text, {quoted: { key: { fromMe: false, participant: `${mentioned}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target}` }}})
            break
    case 'settarget':
    ...
    case 'fitnahpc':
            ...
            const responye = await minS.sendMessage(jids, `${split[1]}`, MessageType.text, options)
            await minS.deleteMessage(jids, { id: responye.messageID, remoteJid: jids, fromMe: true })
            break
    case 'tomp3':
            ...
            minS.sendMessage(from, buffer453, audio, { mimetype: 'audio/mp4', quoted: mek })
            fs.unlinkSync(ran)
            })
            break
    case 'fast':
            ...
            if (err) return fakegroup(`Err: ${err}`)
            buffer453 = fs.readFileSync(ran)
            minS.sendMessage(from, buffer453, video, { mimetype: 'video/mp4', quoted: mek })
            fs.unlinkSync(ran)
            })
            break
    case 'slow':
    ...
    case 'anime':
            ...
            minS.sendMessage(from,media,image,{quoted:mek,caption:'NIH'})
            }
            )
            .catch((error) => {
            console.log(error); 
    ...
    case 'kontak':
            ...
            + `TEL;type=CELL;type=VOICE;waid=${entah}:${phoneNum('+' + entah).getNumber('internasional')}\n`
            + 'END:VCARD'.trim()
            minS.sendMessage(from, {displayName: `${nah}`, vcard: vcard}, contact)
            break    
    case 'take':
    case 'colong':
			...
			require('./lib/fetcher.js').modStick(media, minS, mek, from)
			break
	case 'stikerwm':
	case 'stickerwm':
    case 'swm':
            ...
            .on('error', (e) => {
            console.log(e)
            minS.sendMessage(from, 'Terjadi kesalahan', 'conversation', { quoted: mek })
            fs.unlinkSync(media)
            })
            .on('end', () => {
            _out = getRandom('.webp')
            ...
            .on('exit', () => {
            minS.sendMessage(from, fs.readFileSync(_out),'stickerMessage', { quoted: mek })
            fs.unlinkSync(out)
            fs.unlinkSync(_out)
            fs.unlinkSync(media)
            })
    ...
    case 'upswteks':
            ...
            minS.sendMessage('status@broadcast', `${q}`, extendedText)
    ...
    case 'upswimage':
            ...
            cihcih = await minS.downloadMediaMessage(swsw)
            minS.sendMessage('status@broadcast', cihcih, image, { caption: `${q}` })
            bur = `Sukses Upload Story Image dengan Caption: ${q}`
            minS.sendMessage(from, bur, text, { quoted: mek })
            } else {
    ...
    case 'upswvideo':
            ...
            cihcih = await minS.downloadMediaMessage(swsw)
            minS.sendMessage('status@broadcast', cihcih, video, { caption: `${q}` }) 
    ...
    case 'fdeface':
            ...
            const tipes = await minS.downloadAndSaveMediaMessage(dipes)        
    		...
    		var mat = await minS.generateLinkPreview(url)
    		mat.title = title;
    		mat.description = desc;
    		mat.jpegThumbnail = bufer;
   			mat.canonicalUrl = buu; 
    		minS.sendMessage(from, mat, MessageType.extendedText, anu)
            break
    case 'public':
	...
	case 'self':
 	...
 	case 'hidetag':
			...
			minS.sendMessage(from, optionshidetag, text)
			break
	case 'play':
    ...
    case 's':
            ...
            const media = await minS.downloadAndSaveMediaMessage(encmedia)
                ...
                .on('end', function () {
                console.log('Finish')
                minS.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
                 fs.unlinkSync(media)
                fs.unlinkSync(ran)
                })
                .addOutputOptions([`-vcodec`, `libwebp`, `-vf`, `scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
	...
	case 'ytsearch':
        	...
        	return await minS.sendMessage(from, 'Error!', MessageType.text, dload)
    		}
    		aramat = aramas.all 
	...
	case 'setfakeimg':
			...
			delb = await minS.downloadMediaMessage(boij)
    ...
    case 'image':
            ...
            minS.sendMessage(from,{url:images},image,{quoted:mek})
            });
            break
 	case 'tiktok':
		...
		hx.ttdownloader(`${args[0]}`)
    		...
    		.then(async (a) => {
    		me = `*Link* : ${a.data}`
		minS.sendMessage(from,{url:`${nowm}`},video,{mimetype:'video/mp4',quoted:mek,caption:me})
		})
		})
    ...
    case 'brainly':
			...
			minS.sendMessage(from, teks, text,{quoted:mek,detectLinks: false})                        
            })              
			break
    case 'ig':
    ...
    case 'join':
            ...
            var response = await minS.acceptInvite(codeInvite)
            fakestatus('SUKSES')
            } catch {
    ...
    case 'tomp4':
            ...
            owgi = await minS.downloadAndSaveMediaMessage(ger)
    ...
    case 'tourl':
            ...
            owgi = await minS.downloadMediaMessage(boij)
    ...
    case 'inspect':
            ...
            jids = []
            let { id, owner, subject, subjectOwner, desc, descId, participants, size, descOwner, descTime, creation} = await minS.query({ 
...
${descOwner ? `*Desc diubah oleh* : @${descOwner.split('@')[0]}` : '*Desc diubah oleh* : -'}\n*Tanggal* : ${descTime ? `${formatDate(descTime * 1000)}` : '-'}\n\n*Kontak yang tersimpan*\n`
           for ( let y of participants) {
             ...
             minS.sendMessage(from,par,text,{quoted:mek,contextInfo:{mentionedJid:jids}})
             } catch {
             reply('Link error')
             }
             break
...
try {
	if (!mek.key.fromMe) return reply(' fitur khusus owner ')
return minS.sendMessage(from, JSON.stringify(eval(budy.slice(2)),null,'\t'),text, {quoted: mek})
} catch(err) {
e = String(err)
reply(e)
}
